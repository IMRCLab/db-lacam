...
void tdbastar(
    dynobench::Problem &problem, Options_dbastar options_dbastar,
    Trajectory &traj_out, const std::vector<Constraint> &constraints,
    Out_info &out_info, size_t &robot_id, bool reverse_search,
    ...
    ompl::NearestNeighbors<std::shared_ptr<AStarNode>> **heuristic_result)
{
  ...
  while (!stop_search())
  {
    ...
    for (size_t i = 0; i < lazy_trajs.size(); i++)
    {
      ...
      bool motion_valid = check_lazy_trajectory(
          ...
          constraints, best_node->gScore, options_dbastar.delta,
      ...
      time_bench.time_hfun +=
          timed_fun_void([&]
                         { hScore = h_fun->h(tmp_node->state_eig); });
      assert(hScore >= 0);
  ...
  if (status == Terminate_status::SOLVED)
  {
    ...
    for (const auto &constraint : constraints)
    {
      ...
      assert(dist > options_dbastar.delta);
    }
  }

  out_info.solved = status == Terminate_status::SOLVED;
  out_info.cost = traj_out.cost;
  out_info.time_search = time_bench.time_search;
  out_info.data = time_bench.to_data();
  out_info.data.insert(std::make_pair(
      "terminate_status", terminate_status_str[static_cast<int>(status)]));
  out_info.data.insert(
      std::make_pair("solved", std::to_string(bool(out_info.solved))));
  out_info.data.insert(
