...
class Animation:
  def __init__(self, filename_env, filename_result = None, filename_output = None, filename_expanded_trajs = None):
    ...
    # to visualize the expanded trajs
    if filename_expanded_trajs is not None:
      with open(filename_expanded_trajs, 'r') as motions_file:
        motions = yaml.safe_load(motions_file)
      self.trajs = motions["trajs"]
      self.N_trajs = len(self.trajs)
    # else:
        # raise ValueError("Filename for expanded trajectories is None.")

      # Extract every 10th trajectory
      self.filtered_trajs = [self.trajs[i]["states"] for i in range(0, self.N_trajs, 10)]
      self.max_states = max(len(traj) for traj in self.filtered_trajs)  # Max trajectory length
      
      # Initialize plot
      self.lines = []
      for _ in self.filtered_trajs:
          line, = self.ax.plot([], [], color=".5", alpha=0.5)  # Create empty lines
          self.lines.append(line)

      # Set axis limits (adjust dynamically based on data)
      all_X = [s[0] for traj in self.filtered_trajs for s in traj]
      all_Y = [s[1] for traj in self.filtered_trajs for s in traj]
  ...
  def animate_func(self, i):
    elements = []
    # Phase 1: Animate the trajectory lines first
    # if len(self.filtered_trajs) > 1:
    #   for traj, line in zip(self.filtered_trajs, self.lines):
    #     X = [s[0] for s in traj[:i+1]]
    #     Y = [s[1] for s in traj[:i+1]]
    #     line.set_data(X, Y)
    ...
    for k, robot in enumerate(self.result["result"]): # for each robot
      if i >= len(robot["states"]):
        ...
        elif self.robot_types[k] == 'integrator1_2d_v0':
            pos = state
            xy = np.asarray(pos)
            self.robot_patches[k][0].center = xy
            t = matplotlib.transforms.Affine2D().rotate_around(
