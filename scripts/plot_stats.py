...
class Report:
  def __init__(self, filename, trials, T, dt):
    ...
    cmap = get_cmap("Dark2")
    self.alg_dict = {
      'db-cbs': {'idx': 0, 'color': cmap.colors[0], 'name': 'db-CBS'},
      'db-ecbs': {'idx': 1, 'color': cmap.colors[1], 'name': 'db-ECBS'},
      'db-pibt': {'idx': 2, 'color': cmap.colors[2], 'name': 'db-PIBT'},
      'db-lacam': {'idx': 3, 'color': cmap.colors[3], 'name': 'db-LaCAM'},
    }
    self.color_dict = {
      'db-cbs': cmap.colors[0],
      'db-ecbs': cmap.colors[1],
      'db-pibt': cmap.colors[2],
      'db-lacam': cmap.colors[3],
    }
    self.trials = trials
    self.T = T
    self.dt = dt
